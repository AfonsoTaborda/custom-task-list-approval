...
async function getSimilarGithubCommentId(pullRequestComments) {
    ...
    if (pullRequestComments.length != 0) {
        for (let comment of pullRequestComments) {
            console.log(`Comparing ${comment.body.trim()} \n With: ${inputs.commentTaskList.trim().split(";")} = ${comment.body.trim().includes(inputs.commentTaskList.trim().split(";"))}`)
            if (comment.body.trim().includes(inputs.commentTaskList.trim().split(";")) || inputs.commentTaskList.trim().split(";").includes(comment.body.trim())) {
