...
async function getSimilarGithubCommentId(pullRequestComments) {
            ...
            for (task of inputs.commentTaskList.split(";")) {
                while ((match = CHECK_LIST_REGEX.exec(comment.body)) !== null) {
                    if(match[2].trim() === task.trim()) {
                        similarTaskCount++;
                        console.log(`Same task has been found form the comment ${match[2]} and the input ${task}`);
                    }
                }
            }
            console.log(`Input comment task length: ${inputs.commentTaskList.split(";").length - 1} and similarTaskCount ${similarTaskCount}`);
    ...
    for (let [key,value] of mapCommentSimilar) {
        if (value > 1 && key !== similarCommentId) {
