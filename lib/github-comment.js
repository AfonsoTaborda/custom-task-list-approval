...
async function getSimilarGithubCommentId(pullRequestComments) {
    ...
    if (pullRequestComments.length != 0) {
        for (let comment of pullRequestComments) {
            ...
            console.log(`Input comment task length: ${inputs.commentTaskList.trim().split(";").length} and similarCommentCount ${similarCommentCount}`);
            if(similarTaskCount === inputs.commentTaskList.split(";").length) {
                console.log();
